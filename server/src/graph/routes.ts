...
router.get('/:providerId/cached', (req: Request, res: Response) => {
  ...
  const row = db.prepare(
    'SELECT graph_data, resource_types, scanned_at FROM cached_graphs WHERE user_id = ? AND provider_id = ?'
...
router.get('/:providerId', async (req: Request, res: Response) => {
  ...
  const db = getDb();
  db.prepare(`
    INSERT INTO cached_graphs (user_id, provider_id, resource_types, graph_data, scanned_at)
    VALUES (?, ?, ?, ?, datetime('now'))
    ON CONFLICT(user_id, provider_id) DO UPDATE SET
      resource_types = excluded.resource_types,
      graph_data     = excluded.graph_data,
      scanned_at     = excluded.scanned_at
  `).run(user.id, providerId, resourceTypes.join(','), JSON.stringify(graph));
